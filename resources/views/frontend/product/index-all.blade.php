            ...
            @if (count($categories) != 0)
                <ul>
                    @foreach ($categories as $category)
                        <li><a href="{{ route('category.product', $category->id) }}">{{ $category->title }}</a>
                        </li>
                    @endforeach
                </ul>
            @endif
            ...
            @foreach ($categories as $category)
                <div class="row">
                    @if (count($category->products) != 0)
                        <p class="dash-title"><span class="dash-title-text">{{ $category->title }}</span></p>
                        <a href="{{ route('category.product', $category->id) }}" class=" view-more">View More</a>
                            ...
                            <div class="col-lg-6 col-md-6 col-xl-3 product-margin">
                                <div class="product-fruit-wrapper mb-60">
                                    ...
                                    <div class="product-fruit-content text-center">
                                        ...
                                        <span>Rs. {{ $product->minPrice() }} - {{ $product->maxPrice() }} <p style="font-size: 10px;display:inherit">6 Canas</p></span>
